...
LIB_NAME="libbc-bytewords-jni.dylib"
OUT_DIR=src/main/libs
JNI_MD_DIR="darwin"
if ! is_osx; then
  LIB_NAME="libbc-bytewords-jni.so"
  JNI_MD_DIR="linux"
fi
BUILD_LOG_DIR="log"
...
build_bc_bytewords() {
  pushd "$PARENT_ROOT_DIR/deps/bc-bytewords"
  ./configure
  make clean
  make CFLAGS=-fPIC check
  popd
...
build_jni() {
  ...
  $CC -I"$JAVA_HOME/include" \
    ...
    -I"$PARENT_ROOT_DIR/deps/bc-bytewords/src" \
    -fexceptions -frtti -shared -fPIC \
    src/main/jniLibs/*.c \
    "$PARENT_ROOT_DIR"/base-jni/*.c \
    "$PARENT_ROOT_DIR"/deps/bc-bytewords/src/libbc-bytewords.a \
